...
class TokenFilterTest {
    ...
    void shouldSetAuthContextAndPassFilterWhenAccessTokenIsValid() throws IOException, ServletException {
        ...
        when(jwtParser.getIdFrom(VALID_ACCESS_TOKEN)).thenReturn(id);
        when(jwtParser.getRoleFrom(VALID_ACCESS_TOKEN)).thenReturn(role);
    ...
    void shouldReissueTokensWhenAccessTokenIsExpiredAndRefreshTokenIsValid() throws IOException, ServletException {
        ...
        when(jwtParser.getIdFrom(VALID_REFRESH_TOKEN)).thenReturn(1L);
        when(jwtParser.getRoleFrom(VALID_REFRESH_TOKEN)).thenReturn(Role.MEMBER);

        when(jwtProvider.createAccessToken(anyLong(), any(Role.class), any())).thenReturn(REISSUED_ACCESS_TOKEN);
        when(jwtProvider.createRefreshToken(anyLong(), any(Role.class), any())).thenReturn(REISSUED_REFRESH_TOKEN);
    ...
    void shouldInvalidateTokenAndReturnUnauthorizedIfRefreshTokenIsExpired() throws IOException, ServletException {
        ...
        when(jwtParser.isExpired(EXPIRED_REFRESH_TOKEN)).thenReturn(true);
