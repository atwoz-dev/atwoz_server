...
**규칙 2 - Spring Boot 애플리케이션 포트 (로드 밸런서에서)**
...
인터넷 사용자
  ↓ HTTPS (443)
ALB (deepple-prod-alb-sg) ← HTTP 80, HTTPS 443 처리
  ↓ HTTP (8080)
EC2 (deepple-prod-app-sg) ← 8080만 열면 됨
```

- ALB에서 HTTP 80, HTTPS 443 처리 (SSL 종료)
- EC2는 ALB에서만 8080 포트로 요청 받음
- **EC2에 HTTP/HTTPS 포트를 열 필요 없음** (보안 강화)
...
AWS 보안 그룹은 **Stateful Firewall**입니다:

```
EC2 → (아웃바운드) → Docker Hub 요청
Docker Hub → (응답) → EC2 ← 자동 허용! ✅
```

- **연결 상태 추적**: 나간 요청의 응답은 인바운드 규칙 없이도 자동 허용
- **아웃바운드만 허용하면**: Docker pull, RDS 연결, API 호출 모두 가능
- **예시**:
    - EC2 → Docker Hub (443 포트): 아웃바운드 규칙으로 허용
    - Docker Hub → EC2 (응답): 인바운드 규칙 없어도 자동 허용 (연결 추적)
    - EC2 → RDS (3306 포트): 아웃바운드 허용 + RDS 보안 그룹 인바운드 허용 필요

💡 **네트워크 ACL과의 차이**:

- **보안 그룹**: Stateful (응답 자동 허용)
- **Network ACL**: Stateless (인바운드/아웃바운드 독립적으로 관리)
